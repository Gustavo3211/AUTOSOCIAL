...
import { CommentItem, type CommentData as CommentType } from "@/components/CommentItem"
...
export default function PostDetail() {
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      ...
      if (!data) return
      const normalizedUser = normalizeRelation<{ username: string; is_premium?: boolean; avatar_url?: string }>(
        data.User,
      )
  ...
  useEffect(() => {
    ...
    async function checkInitialLike() {
      try {
        ...
        if (error) {
          console.log("[PostDetail] Erro ao verificar like:", error.message)
          setIsLiked(false)
        } else {
          setIsLiked(!!data)
        }
      } catch (err) {
        console.log("[PostDetail] Timeout ou erro ao verificar like:", err instanceof Error ? err.message : err)
        setIsLiked(false)
      }
    }
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[PostDetail] Erro ao curtir:", error.message)
        setIsLiked(wasLiked)
        setLikeCount((c) => (wasLiked ? c + 1 : c - 1))
      } else {
      ...
      console.error("[PostDetail] Exceção ao curtir:", err)
      setIsLiked(wasLiked)
      setLikeCount((c) => (wasLiked ? c + 1 : c - 1))
    }
  }
  ...
  const handleCommentSubmit = async () => {
    ...
    if (!insertError) {
      ...
      setReplyingTo(null)
      await fetchComments()
      const { data: newCommentCount, error: rpcError } = await supabase.rpc("update_comment_count", {
        post_id_input: post.id,
      })
      if (!rpcError) {
        ...
        <div className="space-y-3 pb-6">
          ...
          {commentsList.length > 0 ? (
            commentsList.map((comment) => (
              <CommentItem key={comment.id} comment={comment} onStartReply={handleStartReply} />
            ))
          ) : (
